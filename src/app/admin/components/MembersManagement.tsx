...
const MembersManagement: React.FC<MembersManagementProps> = ({
  ...
  const [confirmRemove, setConfirmRemove] = useState<Team | null>(null);
  ...
  const handleSearch = async (query: string) => {
    ...
    setSearching(true);
    try {
      const results = await adminService.searchTeams(query, activeAgeCategory);
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            {activeAgeCategory.replace("_", "-")} • {teams.length} Registered
          </p>
        </div>
            ...
            {searchResults.length > 0
              ...
              ? (
                <div className="text-center py-8 text-gray-400 text-sm bg-white rounded-xl border border-dashed border-gray-200">
                  No teams found matching &quot;{searchQuery}&quot; in{" "}
                  {activeAgeCategory.replace("_", "-")}.
                </div>
              )
              : null}
          </div>
        ...
        {teams.length === 0 ? (
          <div className="text-center py-16 px-6 text-gray-400 bg-gray-50/50 rounded-2xl border-2 border-dashed border-gray-100">
            ...
            <p className="text-sm mt-2 max-w-xs mx-auto">
              Add teams to start managing the tournament for{" "}
              {activeAgeCategory.replace("_", "-")}.
            </p>
          </div>
        ) : (
              ...
              <div className="flex items-center gap-5">
                ...
                <div>
                  ...
                  <div className="text-xs text-gray-400 font-medium flex items-center gap-2 mt-0.5">
                    <span className="bg-gray-100 px-2 py-0.5 rounded text-gray-500 font-semibold">
                      {team.ageCategory.replace("_", "-")}
                    </span>
                    {team.coach && (
                      <span>
                        • Coach:{" "}
                        <span className="text-gray-600">{team.coach}</span>
                      </span>
                    )}
                  </div>
                </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl shadow-2xl max-w-sm w-full p-6 animate-in zoom-in-95 duration-200">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 className="font-bold text-gray-900">Remove Team</h3>
                <p className="text-xs text-gray-500">This action may affect existing matches</p>
              </div>
            </div>
            <p className="text-sm text-gray-600 mb-6">
              ...
              from this tournament? Matches involving this team might be affected.
            </p>
            <div className="flex gap-3">
              ...
              <button
                onClick={() => handleRemoveTeam(confirmRemove.id)}
