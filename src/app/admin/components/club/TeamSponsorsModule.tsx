...
import { Team, Sponsor, SponsorTier, TeamSponsor } from "@/types/admin";
import { adminService } from "@/services/adminService";

interface TeamSponsorsModuleProps {
  team: Team;
  onRefresh: () => void;
}

export default function TeamSponsorsModule({ team, onRefresh }: TeamSponsorsModuleProps) {
  const [sponsors, setSponsors] = useState<TeamSponsor[]>(team.sponsors || []);
  ...
  useEffect(() => {
    // Update local state if team prop changes
  ...
  // Filter available sponsors that are not already assigned to this team
  ...
  const handleAssignSubmit = async (e?: React.FormEvent | React.MouseEvent) => {
    ...
    try {
      await adminService.assignSponsorToTeam(
        selectedSponsorId,
        team.id,
        selectedTier
      );
      ...
      if (assignedSponsor) {
        setSponsors(prev => [...prev, { sponsorId: selectedSponsorId, teamId: team.id, tier: selectedTier, sponsor: assignedSponsor }]);
      }
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
      ...
      setError(err.message || "Failed to assign sponsor");
    } finally {
  ...
  const handleRemove = async (sponsorId: string) => {
    if (!confirm("Are you sure you want to remove this sponsor from the team?")) return;

    setLoading(true);
    setError(null);

    try {
      await adminService.removeSponsorFromTeam(sponsorId, team.id);
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden mb-6">
      {/* Header */}
      <div className="px-6 py-5 border-b border-gray-100 bg-linear-to-r from-gray-50 to-white flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Manage sponsors distinctively linked to <span className="font-semibold text-gray-700">{team.name}</span>
          </p>
        </div>
        <div className="bg-indigo-50 text-indigo-700 space-x-1 font-bold px-4 py-1.5 rounded-full shadow-inner text-sm border border-indigo-100">
      ...
      <div className="p-6 space-y-8">
        ...
        <div>
          ...
          {loading ? (
            ...
            <div className="text-center py-10 bg-gray-50/50 rounded-xl border-2 border-dashed border-gray-200 transition-colors hover:border-indigo-300 hover:bg-gray-50">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {sponsors.map((ts) => (
                <div key={ts.sponsorId} className="group relative bg-white border border-gray-200 rounded-xl p-4 shadow-sm hover:shadow-md hover:border-indigo-200 transition-all flex items-center">
        ...
        <div className="bg-linear-to-b from-indigo-50/50 to-white rounded-xl p-6 border border-indigo-100 shadow-sm">
          <h4 className="text-sm font-bold text-indigo-900 uppercase tracking-wider mb-5 flex items-center gap-2">
          ...
          <div className="flex flex-col md:flex-row gap-5 items-end">
            <div className="flex-1 w-full">
              ...
              <div className="relative">
                 <select
                   className="w-full text-sm rounded-xl border border-gray-300 p-3 text-gray-700 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 outline-none bg-white appearance-none transition-all shadow-xs"
            ...
            <button
              ...
              className="w-full md:w-auto px-6 py-3 bg-indigo-600 hover:bg-indigo-700 active:bg-indigo-800 focus:ring-4 focus:ring-indigo-200 text-white text-sm font-bold rounded-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 shadow-sm"
            >
              {isAssigning ? (
                 <>
          ...
          {unassignedSponsors.length === 0 && availableSponsors.length > 0 && (
            <p className="text-xs text-orange-600 mt-3">All existing sponsors are already assigned to this team.</p>
          )}
