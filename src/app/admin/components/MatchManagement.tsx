...
import { FaCalendar, FaEdit, FaTrash, FaCheck, FaTimes, FaPlus, FaFileAlt } from "react-icons/fa"; // Added FaFileAlt
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 flex flex-col h-full">
      <div className="flex justify-between items-center mb-8">
        <div>
            ...
            <p className="text-sm text-gray-500 mt-1">{activeAgeCategory.replace('_', '-')} • {matches.length} Total</p>
        </div>
        
        {!isCreating && !editingMatch && (
            <button 
                onClick={() => setIsCreating(true)}
                className="px-5 py-2.5 bg-blue-600 text-white rounded-xl hover:bg-blue-700 shadow-lg shadow-blue-200 transition-all flex items-center gap-2 text-sm font-semibold transform hover:-translate-y-0.5"
            >
