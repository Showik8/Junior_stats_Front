...
const CreateSponsorForm = ({ onSuccess, onCancel }: CreateSponsorFormProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await adminService.createSponsor({
        name,
        logoUrl,
      ...
      console.error(err);
      setMessage({ type: "error", text: err.message || "Failed to create sponsor" });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 bg-white p-8 rounded-xl shadow-lg border border-gray-100 animate-in fade-in slide-in-from-bottom-4">
      <div className="flex justify-between items-center border-b border-gray-100 pb-4 mb-6">
        <div>
          <h3 className="text-2xl font-bold text-gray-800">Create New Sponsor</h3>
          <p className="text-gray-500 text-sm mt-1">Register a global partner or sponsor.</p>
        </div>
